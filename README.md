...
## Live meeting assistant (Droker Live)

`/meeting` is a single screen meant to sit on the meeting-room display. It listens to the conversation and, when someone asks about a cost, an owner, a deadline or a result, puts the matching knowledge card on screen: figures first, the answer from that angle, then who is responsible, what happened before, and the sources.

- **Router:** every final utterance goes to [TypeSafe Jev](https://docs.typesafe.ai/) through a server function (`src/server/meeting-assist.ts`). One call asks three questions against the same state: `needs_info` (noul: is somebody asking for a fact), `topic` (choice over the knowledge cards plus `none`) and `facet` (choice: cost, owner, deadline, status, history, options). The card shows when `needs_info` and the topic confidence clear the thresholds in `src/server/relevance.ts`; a topic that is only mentioned lands in the "Wspomniano" tray instead.
...
- **Input:** two scripted board meetings (`src/demo/knowledge.ts`, `SCENARIOS`) with autoplay or step-by-step (`Space`), the browser microphone (Web Speech API, `pl-PL`, Chrome), or a text box for typing what was said.
- **Cards:** `KNOWLEDGE_CARDS` in `src/demo/knowledge.ts` flatten the company graph into six topics (Pipedrive, Alfa, Beta Testing, senior backend offer, staging hosting, e-Doręczenia connector). Each has answers per facet, key figures, people, a timeline, open items and source ids from the demo seed.
...
Copy `.env.example` to `.env` and set either `TYPESAFE_API_KEY` (direct, api.typesafe.ai) or `OPENROUTER_API_KEY` (OpenRouter's `/api/alpha/decisions` endpoint, model `typesafe/jev-1.13`; an `sk-or-` key in `TYPESAFE_API_KEY` is detected too). `npm run jev:eval` replays every scripted line through the live model and prints the routing decision, confidence, latency and a pass/fail against the scenario's expected card. Last run: 20/20 cards correct, median 350 ms via OpenRouter.
