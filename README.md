...
Anything else said to Bolek is an **open question over the whole company graph**: `src/server/graph-answer.ts` puts every node and edge of the SQLite graph (66 nodes, 64 edges in the demo snapshot) plus precomputed spend totals and the best-matching source passages (full-text search + graph neighbourhood) into one prompt, and a fast model via OpenRouter (`ANSWER_MODEL`, default `anthropic/claude-haiku-4.5`) returns a Polish answer as JSON: headline figure, one to three sentences, bullets and the `[Cn]` citations it used. The answer card shows the quoted passages with file path and date. Questions the graph cannot answer come back with `found=false` and a "brak w danych" pill instead of a guess. Jev runs in parallel: a person match replaces the answer card with the profile. Topic cards only surface in the always-listening mode; when Bolek is addressed, the graph answer is the single source of truth, so the Droker-seeded cards and the Aster Systems graph never appear side by side. `npm run ask "…"` asks from the terminal (about 3 s per question).
